...
export default function GalleryPageClient({
  ...
  initialFolders: { name: string; path: string; image: string }[];
}) {
  const [folderPreviews, setFolderPreviews] = useState(initialFolders);
          ...
          {folderPreviews.map((folder) => (
            ...
            >
              <div className="relative w-full h-48">
                <Image
                  src={
                    isCloudinaryUrl(folder.image)
                      ? folder.image
                      : getCloudinaryUrl(folder.image, {
                          width: 400,
                          quality: 80,
                        })
                  }
